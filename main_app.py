...
#------------Show Api Data-----------------------
            band_data=gfit.getData()
            combine_band_data = [band_data['heart_minutes'][0]['value'],band_data['steps_count'][0]['value'],band_data['calories_expended'][0]['value'],band_data['weight'][0]['value']]
            #------------------------------------------------


            return render_template('index.html', doctor=doctor, len=len(doctor), patient=account, c=c, status=status, len1=len1, queries=queries, len2=len(queries),data_view=temp_data,bdata=combine_band_data)
        else:
